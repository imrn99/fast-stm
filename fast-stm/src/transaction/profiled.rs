...
use super::{RegisterType, TransactionGuard};
...
pub struct Transaction<'a> {
    ...
    vars: RegisterType,
    tallies: &'a mut TransactionTallies,
}
...
impl Transaction<'_> {
    ...
    fn new(tallies: &mut TransactionTallies) -> Transaction<'_> {
        Transaction {
    ...
    pub fn with<T, F>(tallies: &mut TransactionTallies, f: F) -> T
    where
        F: Fn(&mut Transaction) -> StmClosureResult<T>,
    {
        match Transaction::with_control(tallies, |_| TransactionControl::Retry, f) {
            Some(t) => t,
            None => unreachable!(),
        }
    }
    ...
    pub fn with_control<T, F, C>(
        tallies: &mut TransactionTallies,
        mut control: C,
        f: F,
    ) -> Option<T>
    where
        ...
        C: FnMut(StmError) -> TransactionControl,
    {
        ...
        // the log
        let mut transaction = Transaction::new(tallies);

        // loop until success
        loop {
            transaction.tallies.n_attempts += 1;
            // run the computation
            match f(&mut transaction) {
                ...
                Err(e) => {
                    match e {
                        StmError::Failure => {
                            transaction.tallies.n_error += 1;
                        }
                        StmError::Retry => {
                            transaction.tallies.n_retry += 1;
                        }
                    }
    ...
    pub fn with_err<T, F, E>(tallies: &mut TransactionTallies, f: F) -> Result<T, E>
    where
        F: Fn(&mut Transaction) -> TransactionClosureResult<T, E>,
    {
        let _guard = TransactionGuard::new();
        ...
        loop {
            ...
            match f(&mut transaction) {
                ...
                Err(e) => match e {
                    ...
                    // retry
                    TransactionError::Stm(err) => match err {
                        StmError::Failure => {
                            transaction.tallies.n_error += 1;
                        }
                        StmError::Retry => {
    ...
    fn combine(&mut self, other: RegisterType) {
        // combine reads
...
mod test {
    ...
    fn read() {
        ...
        let mut log = Transaction::new(&mut t);
    ...
    fn transaction_simple() {
        let mut t = TransactionTallies::default();
        let x = Transaction::with(&mut t, |_| Ok(42));
        assert_eq!(x, 42);
    }
    ...
    fn transaction_read() {
        ...
        let mut t = TransactionTallies::default();
        let x = Transaction::with(&mut t, |trans| read.read(trans));
    ...
    fn transaction_with_control_abort_on_single_run() {
        ...
        let mut t = TransactionTallies::default();
        let x =
            Transaction::with_control(&mut t, |_| TransactionControl::Abort, |tx| read.read(tx));
    ...
    fn transaction_write() {
        ...
        let mut t = TransactionTallies::default();
        Transaction::with(&mut t, |trans| write.write(trans, 0));
    ...
    fn transaction_nested_fail() {
        let mut t = TransactionTallies::default();
        Transaction::with(&mut t, |_| {
            let mut tt = TransactionTallies::default();
            Transaction::with(&mut tt, |_| Ok(42));
            Ok(1)
        });
    }
}
